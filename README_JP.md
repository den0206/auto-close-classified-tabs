...
エクスプローラー側の装飾も併せて調整するため、ウィンドウごとに一度、案内が出ます。自分で実行しても構いません。
...
言語バーに使っているのと同じ色 — に合わせています。`.ts` は TypeScript の青になるので、対応を
覚え直す必要がありません。Linguist の値は小さな丸ドット用に選ばれているため、タブのラベルとして
読めるように**明度だけ**をテーマごとに寄せています(色相と彩度はそのまま)。

**ただし赤系と黄系は使いません。** タブの文字色は VS Code がエラー(赤)と警告(黄)を出すのと
同じ場所に出るため、種別の色をそこへ置くと問題の有無と読み違えます。Linguist で赤や黄・橙に
なる種別(`javascript` `yaml` `ruby` `swift` `cfamily` `rust` `jvm` `sql` `env` `toml` `html`)は、
緑〜青〜紫〜ピンクの寒色側へ色相を振り直してあります。`diff` `test` `generated` は言語ではなく
状態で Linguist に対応する色が無いため、独自の色です。`json` も例外で、Linguist の色がほぼ黒
(`#292929`)でタブ上は `generated` のグレーと見分けがつかないため、従来の黄緑を残しています。
...
設定を書き込むコマンドを実行したことがある場合は、先に対になるコマンドを実行してください。

> **Auto Close Classified Tabs: 色とバッジの表示を既定に戻す**
> **Auto Close Classified Tabs: タブ名から種別アイコンを取り除く**

VS Code は拡張にアンインストールのフックを渡さないため、書き込んだ設定を自動では片づけられません。
残したままにすると、理由を説明するものが何も無い状態で
**エクスプローラーの Git の色と `M` / `U` バッジが消えたまま**になり、**タブ名の絵文字も残ります**。
`settings.json` から `workbench.editor.decorations.*` `explorer.decorations.*`
`workbench.editor.customLabels.patterns` を手で消しても同じです。
