...
| 保存先 | 方針 |
|---|---|
| `globalState` / `workspaceState` | **一切使わない**。装飾の案内を出したかどうかもウィンドウ変数のままにする(新しいウィンドウで聞き直されるのは、データを残さないことの対価) |
| ファイル書き込み | **しない** |
| `secrets` | 使わない |
| `settings.json` | 読むのは常時。書き込むのは **ユーザーがコマンドを実行したときだけ**(`enableTabDecorations` / `applyLabelIcons`)で、どちらにも取り消すコマンドがある |

→ **拡張が抱えるデータ**はゼロ。LRU も案内の既読フラグも揮発メモリのみで、ウィンドウを閉じれば消える。
→ **ユーザーの `settings.json` に書いた分は残る。** VS Code はアンインストールのフックを拡張に渡さないので、
自動では片づけられない。打てる手は「書き込むコマンドには必ず取り消すコマンドを対で用意する」「書き込んだ
直後のメッセージで取り消すコマンド名を伝える」「README にアンインストール前の手順を書く」の 3 つで、全部やっている。
...
暖色を捨てると 26 色を色相 255° に詰めることになるので、どれだけ詰まったかを測って見張る。

### 色差は CIEDE2000 で測る(CIE76 では足りなかった)
当初は Lab のユークリッド距離(CIE76)で測り、「最近傍でも ΔE 10〜13 あるから大丈夫」と
判断していた。**これが間違いだった。** CIE76 は青系の差を過大に見積もる。暖色を捨てた結果
いちばん混み合っているのがまさに青帯なので、指標の弱点と palette の弱点が正面から重なる。

CIEDE2000 で測り直すと、light の `typescript` `#2863A4` と `xml` `#0060AC` は **ΔE 1.8** しか
離れていなかった(CIE76 では 7.0 に見えていた)。1.8 は「並べて凝視すれば違うと言える」水準で、
タブの細い文字では実質同色。しかも当たっていたのが TypeScript↔XML/SVG、TypeScript↔Python という
**同じリポジトリで最も同時に開かれる組**だった。dark の `typescript`/`python` も ΔE 3.6。

`scripts/color-distance.js` が CIEDE2000 で全 325 組を測り、`scripts/color-distance.test.js` が
下限を固定する。**色を足す・変えるときにここが落ちたら、その色が近すぎる。**

| 指標 | 下限 | 根拠 |
|---|---|---|
| 色同士の距離 | ΔE2000 **6.0 以上**(実測 dark 6.2 / light 6.3) | これを割ると実機で色の区別が効かなくなる |
| 背景とのコントラスト | **4.5:1 以上**(WCAG AA・通常サイズの文字) | タブラベルは UI テキスト。`generated` だけは「沈ませる」のが仕事なので対象外 |

コントラストの方も後から入れた基準で、入れた時点で 6 色(`csharp` dark、`test` `css` `dart`
`shell` `go` の light)が割っていた。とくに light の `shell` `#57AD1F` は 2.84:1、`dart`
`#00B4AB` は 2.59:1 で、白背景ではほとんど読めない。明度を落として基準内へ入れた
(`shell` は緑帯が `json` `toml` と混むため、読める明度まで下げると深い緑になる)。
...
**実績: ユニット 73 件 / 統合 21 件。ユニットには色差(ΔE2000)とコントラストの下限チェックを含む。**
